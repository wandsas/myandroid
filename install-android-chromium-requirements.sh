...
sudo apt-get install -y openjdk-8-jdk openjdk-11-jdk 				\
  ...
  pulseaudio pulseaudio-utils kvm qemu-user-static                              

# Install Google protobuf (required for android-prepare-vendor):                
pip install --upgrade (--user) protobuf google                                  
# When protoc not function, try to uninstall & reinstall it.                    
pip uninstall (--user) protobuf google && pip install (--user) protobuf google  
...
sudo apt-get install -y libpci3:i386 libx11-xcb1:i386 linux-libc-dev:i386 	 	\
  ...
  binutils-mips64el-linux-gnuabi64 binutils-mipsel-linux-gnu cdbs libffi 	 	\
  ...
  python2-dev python-setuptools rpm ruby snapcraft lib1g						\
